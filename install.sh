...
mkdir -p "$CODEX_HOME/instructions" "$CODEX_HOME/agents"
...
"$PYTHON_BIN" - "$TEMPLATE_ROOT/worktree-multiagent.config.toml.tpl" "$profile_dest" "$escaped_instruction_path" <<'PY'
import pathlib
import sys
